# # Most initial version, uses openai embed
...
# -------------------------------------------------------------------
class OpenAITextEmbedding3Large(Embeddings):
    """
    LangChain Embeddings wrapper for OpenAI text-embedding-3-large.
    ...
    def _embed(self, texts: List[str]) -> List[List[float]]:
        ...
        if not data:
            raise RuntimeError(f"OpenAI embedding call returned no data: {resp}")
...
def infer_symbol_kind_from_content(content: str, symbol_name: Optional[str]) -> str:
    ...
    if symbol_name:
        ...
        if f"_class_ {sn_lower}" in head_lower or f"class {sn}(" in head:
            return "class"
        if f"_function_" in head_lower or f"{sn}(" in head:
...
def build_manim_vector_store(
    ...
    dense_embeddings = OpenAITextEmbedding3Large(
...
def main() -> None:
    load_dotenv()
    collection_name = "manim_docs_hybrid"
    ...
    if REBUILD_INDEX:
        ...
        docs = chunk_docs(docs, chunk_size=2000, chunk_overlap=200)
